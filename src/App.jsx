...
function AgendaSection({ userId, setModal }) {
  ...
  const load = async () => {
    setLoading(true)
    const { data } = await supabase.from("appointments").select("*").eq("user_id", userId).order("date").order("time")
    setAppts(data || [])
    setLoading(false)
  }
  useEffect(() => { load() }, [])
...
function PatientsSection({ userId, setModal, onOpen }) {
  ...
  const load = async () => {
    setLoading(true)
    const { data } = await supabase.from("patients").select("*").eq("user_id", userId).order("name")
    setPatients(data || [])
    setLoading(false)
  }
  useEffect(() => { load() }, [])
...
function AINotesSection({ userId }) {
  ...
  useEffect(() => {
    supabase.from("patients").select("id,name,blood_type,allergies").eq("user_id", userId).order("name").then(({ data }) => setPatients(data || []))
  }, [])
...
function AddPatientForm({ userId, onSave, onClose }) {
  ...
  const save = async () => {
    ...
    setSaving(true)
    await supabase.from("patients").insert({ ...f, user_id: userId })
    onSave(); onClose()
  }
...
function AddApptForm({ userId, onSave, onClose }) {
  ...
  useEffect(() => {
    supabase.from("patients").select("id,name").eq("user_id", userId).order("name").then(({ data }) => setPatients(data || []))
  }, [])
  ...
  const save = async () => {
    ...
    await supabase.from("appointments").insert({ ...f, patient_name: patient?.name || "", user_id: userId })
    onSave(); onClose()
  }
...
function AddHistoryForm({ patientId, userId, onSave, onClose }) {
  ...
  const save = async () => {
    ...
    setSaving(true)
    await supabase.from("clinical_history").insert({ ...f, patient_id: patientId, user_id: userId })
    onSave(); onClose()
  }
...
function AddPhotoForm({ patientId, userId, onSave, onClose }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      await supabase.from("photos").insert({ ...f, patient_id: patientId, user_id: userId, storage_path: fileName, url })
      onSave(); onClose()
    } catch (e) {
      // Save with base64 as fallback
      await supabase.from("photos").insert({ ...f, patient_id: patientId, user_id: userId, url: preview })
      onSave(); onClose()
    }
  }
